...
impl ParsedUri {
    ...
    pub fn to_uri_str(&self) -> Result<String, String> {
...
mod tests {
    ...
    // Reverse conversion tests: ParsedUri::to_uri_str should produce the same string as the builder functions
    ...
    fn test_user_uri_roundtrip() {
        ...
        let reconstructed_uri = parsed.to_uri_str().expect("Failed to convert to URI string");
        assert_eq!(
            original_uri, reconstructed_uri,
            "User URI roundtrip failed"
        );
    }
    ...
    fn test_unknown_resource_to_uri_str_fails() {
        ...
        let parsed = ParsedUri::try_from(uri).expect("Failed to parse URI with unknown resource");
        assert_eq!(parsed.resource, Resource::Unknown);
        let result = parsed.to_uri_str();
        assert!(result.is_err(), "Unknown resource should fail to convert to URI string");
    }
}
